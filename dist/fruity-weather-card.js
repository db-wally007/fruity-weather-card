 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */
const { is: Re, defineProperty: Ie, getOwnPropertyDescriptor: Te, getOwnPropertyNames: He, getOwnPropertySymbols: Pe, getPrototypeOf: Ue } = Object, ht = globalThis, Ot = ht.trustedTypes, Qe = Ot ? Ot.emptyScript : "", ze = ht.reactiveElementPolyfillSupport, K = (e, t) => e, at = { toAttribute(e, t) {
  switch (t) {
    case Boolean:
      e = e ? Qe : null;
      break;
...
const C = 13, B = 11, X = 0.48, Y = 0.375;
function ei(e, t) {
  return {
    north: e + (B - 1) / 2 * Y,
    south: e - (B - 1) / 2 * Y,
...
function Jt(e) {
  return e.nx === C && e.ny === B && Math.abs((e.dlon ?? 0) - X) < 1e-6 && Math.abs((e.dlat ?? 0) - Y) < 1e-6;
}
...
async function ai(e, t) {
  ...
  for (let b = 0; b < B; b++)
    for (let u = 0; u < C; u++)
      s.push((e + (b - (B - 1) / 2) * Y).toFixed(4)), a.push((t + (u - (C - 1) / 2) * X).toFixed(4));
  ...
  const p = (b) => (/* @__PURE__ */ new Date(`${b}Z`)).getTime(), h = c[0].hourly.time.map(p), d = c[0].minutely_15.time.map(p), g = (b, u) => Array.from({ length: b }, (x, k) => {
    const v = new Float32Array(C * B);
    for (let f = 0; f < c.length; f++) v[f] = u(c[f], k) || 0;
    return v;
  }), A = {
    fetchedAt: Date.now(),
    lat0: e,
...
function pi(e, t, i, s) {
  ...
  for (let k = 0; k < B; k++)
    for (let v = 0; v < C; v++) {
      const f = (B - 1 - k) * C + v;
      ci(i[f], p.data, (k * C + v) * 4);
    }
  c.putImageData(p, 0, 0);
  const h = document.createElement("canvas");
  h.width = C * 6, h.height = B * 6;
...
var yi = Object.defineProperty, bi = Object.getOwnPropertyDescriptor, O = (e, t, i, s) => {
...
let y = class extends q {
  constructor() {
    super(...arguments), this._hourly = [], this._daily = [], this._mapOpen = !1, this._mapFrame = 0, this._mapPlaying = !1, this._sheetDay = null, this._hourScrub = null, this._sheetAnchor = 0, this._hourlyError = !1, this._hourlyPending = !1, this._mapRange = "12h", this._mapSeen = !1, this._mapT = 0, this._mapLast = 0, this._gridPending = !1, this._gridRetryAt = 0, this._gridBackoff = 0, this._onWrapClick = (e) => {
      const i = [...this.renderRoot.querySelectorAll(".drow")].findIndex((s) => {
        const a = s.getBoundingClientRect();
        return e.clientX >= a.left && e.clientX <= a.right && e.clientY >= a.top && e.clientY <= a.bottom;
      });
      i >= 0 ? this._openDaySheet(i) : this._closeDaySheet();
    }, this._advance = (e) => {
  ...
  _openDaySheet(e) {
    this._hourScrub = null, this._sheetDay = e, this._anchorToRow(e), this._ensureHourly();
  }
  /**
   ...
   */
  _anchorToRow(e) {
    const t = this.renderRoot.querySelector("ha-card"), i = this.renderRoot.querySelectorAll(".drow")[e];
    if (!t || !i) return;
    const s = i.getBoundingClientRect();
    this._sheetAnchor = s.top + s.height / 2 - t.getBoundingClientRect().top;
  }
  _closeDaySheet() {
  ...
  _positionSheet() {
    ...
    if (t.style.top = `${r}px`, i) {
      ...
      i.style.top = `${r + n}px`, i.style.left = `${t.offsetLeft - y.ARROW_W}px`;
    }
  }
  ...
  _sunArc() {
    ...
    if (!e || !t) return;
    const i = 864e5, s = Date.now(), a = 0.2, o = 0.8, r = 26, n = 17, l = 11, c = 100, p = (u) => u >= a && u <= o ? Math.sin(Math.PI * (u - a) / (o - a)) : u < a ? -Math.sin(Math.PI * ((a - u) / (2 * a))) : -Math.sin(Math.PI * ((u - o) / (2 * (1 - o)))), h = (u) => r - u * (u >= 0 ? n : l), d = (u, x, k) => {
      const v = [];
      for (let f = 0; f <= k; f++) {
        const S = u + (x - u) * f / k;
        v.push(`${(S * c).toFixed(2)},${h(p(S)).toFixed(2)}`);
      }
      return `M${v.join(" L")}`;
  ...
  _renderHourly() {
    ...
    const r = this._override("wind_gust"), n = this._overrideUnit("wind_gust", "wind_speed_unit") ?? "km/h", l = Ut[this._weather?.state ?? ""] ?? "", c = l ? `${l} conditions expected for the rest of the day.` + (r !== void 0 ? ` Wind gusts are up to ${_(r)} ${n}.` : "") : "", p = this._tap("hourly");
    return m`
        ...
        <div class="row">
          ${s.map((h) => h.kind === "sun" ? m`
                ...
                <div class="cell">
                  ...
                  <div class="cell-val">${_(h.temp)}°</div>
                </div>`)}
        </div>
      </div>
    `;
  ...
  _renderSheetBody(e, t, i) {
    ...
    const g = e.length, A = (f) => g > 1 ? f / (g - 1) * 100 : 50, b = (f) => (p - f) / h * 100, u = e.map((f, S) => `${A(S)},${b(f.temp)}`).join(" "), x = `0,100 ${u} 100,100`, k = Math.max(1, Math.round(g / 8)), v = this._hourScrub !== null ? e[this._hourScrub] : void 0;
    return m`
      ...
      this._hourScrub = null;
        ...
        <div class="sheet-yaxis">
          ${d.map((f) => m`<span style=${`top:${b(f)}%`}>${_(f)}°</span>`)}
        </div>
      </div>
  ...
  _renderTiles() {
    ...
    return m`
        ...
        <div class="tile" ?tappable=${!!this._tap("precipitation")}
          ...
          <div class="tile-value">${c === void 0 ? "--" : _(c)} mm</div>
        ...
        <div class="tile" ?tappable=${!!this._tap("humidity")}
          ...
          <div class="tile-note">
            ${o !== void 0 ? m`The dew point is ${_(o)}° right now.` : w}
          </div>
        </div>
  ...
  _renderWindTile(e, t, i, s, a) {
    ...
    const c = a !== void 0, p = c ? a + 180 : 0, h = r(p), d = r(p + 180), g = 38, [A, b] = n(g, d), [u, x] = n(g - 3, d), [k, v] = n(24.5, d), [f, S] = n(22.5, h), [Zt, te] = n(30, h), ct = 1.32, wt = (g + 2 + 30) / 2, yt = (g + 2 - 30) / 2 * ct, [ee, ie] = n(wt + yt, h), [bt, vt] = n(wt - yt, h), xt = -Math.sin(h), _t = Math.cos(h), et = 3.4 * ct, se = bt + xt * et, ae = vt + _t * et, oe = bt - xt * et, re = vt - _t * et, ne = (M) => {
