 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */
const { is: Re, defineProperty: Ie, getOwnPropertyDescriptor: Te, getOwnPropertyNames: He, getOwnPropertySymbols: Pe, getPrototypeOf: Ue } = Object, ht = globalThis, St = ht.trustedTypes, Qe = St ? St.emptyScript : "", ze = ht.reactiveElementPolyfillSupport, K = (e, t) => e, at = { toAttribute(e, t) {
  switch (t) {
    case Boolean:
      e = e ? Qe : null;
      break;
...
let j = class extends HTMLElement {
  ...
  static finalize() {
    ...
    if (this.finalized = !0, this._$Ei(), this.hasOwnProperty(K("properties"))) {
      const i = this.properties, s = [...He(i), ...Pe(i)];
...
j.elementStyles = [], j.shadowRootOptions = { mode: "open" }, j[K("elementProperties")] = /* @__PURE__ */ new Map(), j[K("finalized")] = /* @__PURE__ */ new Map(), ze?.({ ReactiveElement: j }), (ht.reactiveElementVersions ??= []).push("2.1.2");
/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
...
const Ne = (e, t) => {
  ...
  let a, o = t === 2 ? "<svg>" : t === 3 ? "<math>" : "", r = V;
  for (let n = 0; n < i; n++) {
    const l = e[n];
    let c, p, h = -1, d = 0;
    for (; d < l.length && (r.lastIndex = d, p = r.exec(l), p !== null); ) d = r.lastIndex, r === V ? p[1] === "!--" ? r = Dt : p[1] !== void 0 ? r = Rt : p[2] !== void 0 ? (Ft.test(p[2]) && (a = RegExp("</" + p[2], "g")), r = I) : p[3] !== void 0 && (r = I) : r === I ? p[0] === ">" ? (r = a ?? V, h = -1) : p[1] === void 0 ? h = -2 : (h = r.lastIndex - p[2].length, c = p[1], r = p[3] === void 0 ? I : p[3] === '"' ? Tt : It) : r === Tt || r === It ? r = I : r === Dt || r === Rt ? r = V : (r = I, a = void 0);
    const g = r === I && e[n + 1].startsWith("/>") ? " " : "";
    o += r === V ? l + je : h >= 0 ? (s.push(c), l.slice(0, h) + Lt + l.slice(h) + R + g) : l + R + (h === -2 ? n : g);
  }
...
class Z {
  constructor({ strings: t, _$litType$: i }, s) {
    ...
    this.parts = [];
    let o = 0, r = 0;
    const n = t.length - 1, l = this.parts, [c, p] = Ne(t, i);
    if (this.el = Z.createElement(c, s), P.currentNode = this.el.content, i === 2 || i === 3) {
    ...
    for (; (a = P.nextNode()) !== null && l.length < n; ) {
      if (a.nodeType === 1) {
        ...
        let h = -1;
        for (; (h = a.data.indexOf(R, h + 1)) !== -1; ) l.push({ type: 7, index: o }), h += R.length - 1;
      }
      o++;
    }
  }
...
class Fe {
  ...
  u(t) {
    ...
    P.currentNode = a;
    let o = P.nextNode(), r = 0, n = 0, l = s[0];
    for (; l !== void 0; ) {
      if (r === l.index) {
        let c;
        l.type === 2 ? c = new tt(o, o.nextSibling, this, t) : l.type === 1 ? c = new l.ctor(o, l.name, l.strings, this, t) : l.type === 6 && (c = new Je(o, this, t)), this._$AV.push(c), l = s[++n];
      }
      r !== l?.index && (o = P.nextNode(), r++);
    }
...
Ke?.(Z, tt), (At.litHtmlVersions ??= []).push("3.3.3");
const qe = (e, t, i) => {
...
const C = 13, O = 11, X = 0.48, Y = 0.375;
function ei(e, t) {
  return {
    north: e + (O - 1) / 2 * Y,
    south: e - (O - 1) / 2 * Y,
...
function Jt(e) {
  return e.nx === C && e.ny === O && Math.abs((e.dlon ?? 0) - X) < 1e-6 && Math.abs((e.dlat ?? 0) - Y) < 1e-6;
}
...
async function ai(e, t) {
  ...
  for (let b = 0; b < O; b++)
    for (let u = 0; u < C; u++)
      s.push((e + (b - (O - 1) / 2) * Y).toFixed(4)), a.push((t + (u - (C - 1) / 2) * X).toFixed(4));
  ...
  if (c.length !== C * O)
    throw new Error(`open-meteo returned ${c.length} of ${C * O} points`);
  const p = (b) => (/* @__PURE__ */ new Date(`${b}Z`)).getTime(), h = c[0].hourly.time.map(p), d = c[0].minutely_15.time.map(p), g = (b, u) => Array.from({ length: b }, (x, _) => {
    ...
    hourly: {
      times: h,
      frames: g(h.length, (b, u) => b.hourly.precipitation[u])
    },
    quarter: {
      times: d,
...
const Kt = "fruity-weather-card:precip-grid:v3", qt = 30 * 6e4;
function oi(e, t) {
  try {
    const i = localStorage.getItem(Kt);
    if (!i) return;
    const s = JSON.parse(i);
    if (Date.now() - s.fetchedAt > qt || Math.abs(s.lat0 - e) > 1e-6 || Math.abs(s.lon0 - t) > 1e-6 || !Jt(s)) return;
    const a = (o) => o.map((r) => Float32Array.from(r));
    return {
      fetchedAt: s.fetchedAt,
      lat0: s.lat0,
...
function li(e, t = "dark") {
  const i = "https://services.arcgisonline.com/ArcGIS/rest/services/Canvas", s = t === "dark" ? "Dark" : "Light", a = e.z, o = 256, r = 2 ** a, n = [], l = Math.floor(e.originX / o), c = Math.floor(e.originY / o), p = Math.floor((e.originX + e.width) / o), h = Math.floor((e.originY + e.height) / o);
  for (let d = c; d <= h; d++)
    if (!(d < 0 || d >= r))
      for (let g = l; g <= p; g++) {
        const A = (g % r + r) % r;
...
function ci(e, t, i) {
  ...
  const a = L[s], o = L[Math.min(s + 1, L.length - 1)], r = o[0] - a[0], n = r > 0 ? Math.min(1, Math.max(0, (e - a[0]) / r)) : 0;
  t[i] = a[1] + (o[1] - a[1]) * n, t[i + 1] = a[2] + (o[2] - a[2]) * n, t[i + 2] = a[3] + (o[3] - a[3]) * n, t[i + 3] = a[4] + (o[4] - a[4]) * n;
}
const di = L.slice(1).map(([e, t, i, s]) => `rgb(${t},${i},${s}) ${((e / 15) ** 0.45 * 100).toFixed(0)}%`).join(", ");
function pi(e, t, i, s) {
...
var yi = Object.defineProperty, bi = Object.getOwnPropertyDescriptor, S = (e, t, i, s) => {
  for (var a = s > 1 ? void 0 : s ? bi(t, i) : t, o = e.length - 1, r; o >= 0; o--)
    (r = e[o]) && (a = (s ? r(t, i, a) : r(a)) || a);
  return s && a && yi(t, i, a), a;
};
const vi = jt(di), xi = "/local/weather-bg", _i = {
...
function Q(e, t) {
  return e ? t && Pt[e] ? Pt[e] : _i[e] ?? "not-available" : "not-available";
}
const Ut = {
  "clear-night": "Clear",
  cloudy: "Cloudy",
...
function Qt(e) {
  if (e <= H[0][0]) return `rgb(${H[0][1].join(",")})`;
  const t = H[H.length - 1];
  if (e >= t[0]) return `rgb(${t[1].join(",")})`;
  for (let i = 0; i < H.length - 1; i++) {
    const [s, a] = H[i], [o, r] = H[i + 1];
    if (e >= s && e <= o) {
      const n = (e - s) / (o - s);
      return `rgb(${a.map((c, p) => Math.round(c + (r[p] - c) * n)).join(",")})`;
    }
  }
  return "#ffffff";
}
...
let y = class extends q {
  constructor() {
    super(...arguments), this._hourly = [], this._daily = [], this._mapOpen = !1, this._mapFrame = 0, this._mapPlaying = !1, this._sheetDay = null, this._hourScrub = null, this._sheetAnchor = 0, this._hourlyError = !1, this._hourlyPending = !1, this._mapRange = "12h", this._mapSeen = !1, this._mapT = 0, this._mapLast = 0, this._gridPending = !1, this._gridRetryAt = 0, this._gridBackoff = 0, this._advance = (e) => {
      const t = Math.max(1, this._mapSeries.frames.length - 1), i = Math.min((e - this._mapLast) / 1e3, 0.25);
      this._mapLast = e, this._mapT = (this._mapT + i / y.FRAME_SECONDS) % t, this._paintMap(), this._syncMapBar();
  ...
  setConfig(e) {
    ...
    this._config = { hourly_hours: 24, daily_days: 10, ...e }, this._mapZoom = y._loadZoom();
  }
  getCardSize() {
    return 14;
  }
  ...
  updated(e) {
    super.updated(e), (e.has("hass") || e.has("_config")) && this.hass && this._config && (this._subscribedTo !== this._config.entity && this._subscribe(), this._config.map && this._watchMapVisible()), this._config?.map && (this._paintMap(), this._syncMapBar(), this._watchMapSize()), this._sheetDay !== null && (this._positionSheet(), this._positionReadout());
  }
  /* ------------------------------------------------ precipitation map ---- */
  /**
  ...
  async _ensureGrid() {
    ...
    if (!(e === void 0 || t === void 0 || this._gridPending) && !(this._grid && Date.now() - this._grid.fetchedAt < qt) && !(Date.now() < this._gridRetryAt)) {
      this._gridPending = !0;
      try {
  ...
  _openDaySheet(e, t) {
    if (t) {
      const i = this.renderRoot.querySelector("ha-card");
      if (i) {
        const s = t.getBoundingClientRect();
        this._sheetAnchor = s.top + s.height / 2 - i.getBoundingClientRect().top;
      }
    }
    this._hourScrub = null, this._sheetDay = e, this._ensureHourly();
  }
  _closeDaySheet() {
    this._sheetDay = null, this._hourScrub = null;
  }
  /**
   ...
   */
  _positionSheet() {
    const e = this.renderRoot.querySelector("ha-card"), t = this.renderRoot.querySelector(".sheet"), i = this.renderRoot.querySelector(".sheet-arrow");
    if (!e || !t) return;
  ...
  _hoursForDay(e) {
    ...
    return !t || !this._hourlyDays ? [] : this._hourlyDays.days.get(wi(new Date(t.datetime))) ?? [];
  }
  get _mapSeries() {
    const e = this._grid;
  ...
  _paintMap() {
    ...
    if (!n) return;
    const l = Math.min(Math.max(this._mapT, 0), n - 1e-6), c = Math.floor(l), p = Math.min(c + 1, n - 1);
    let h = r.frames[c];
    p !== c && ((!this._mapScratch || this._mapScratch.length !== h.length) && (this._mapScratch = new Float32Array(h.length)), h = gi(r.frames[c], r.frames[p], l - c, this._mapScratch)), pi(o, this._grid, h, s);
  }
  /**
  ...
  async _toggleMap() {
    ...
    if (this._mapOpen = !this._mapOpen, this._mapOpen || (this._stopPlayback(), this._mapT = 0, this._mapFrame = 0), await this.updateComplete, this._mapOpen && !e && this._autoPlay(), !e) {
      for (const a of i) {
        const o = s.get(a), r = a.getBoundingClientRect(), n = o.left - r.left, l = o.top - r.top;
        !n && !l || (a.style.transition = "none", a.style.transform = `translate(${n}px, ${l}px)`);
      }
   ...
   */
  get _zoom() {
    return this._mapZoom ?? this._config?.map_zoom ?? y.ZOOM_MIN;
  }
  /**
  ...
  static _loadZoom() {
    try {
      const e = localStorage.getItem(y.ZOOM_KEY);
      if (e === null) return;
      const t = Number(e);
      return Number.isFinite(t) ? Math.min(
        y.ZOOM_MAX,
        Math.max(y.ZOOM_MIN, Math.round(t))
      ) : void 0;
    } catch {
      return;
    }
  ...
  _zoomBy(e) {
    const t = Math.min(
      y.ZOOM_MAX,
      Math.max(y.ZOOM_MIN, this._zoom + e)
    );
    if (t !== this._zoom) {
      this._mapZoom = t;
      try {
        localStorage.setItem(y.ZOOM_KEY, String(t));
      } catch {
      }
    }
  }
  ...
  _timeLabel(e, t = !1) {
    ...
    return m`${i}${s ? m`<span class="ap">${s}</span>` : w}`;
  }
  /**
  ...
  _sunArc() {
    ...
    const i = 864e5, s = Date.now(), a = 0.2, o = 0.8, r = 26, n = 17, l = 11, c = 100, p = (u) => u >= a && u <= o ? Math.sin(Math.PI * (u - a) / (o - a)) : u < a ? -Math.sin(Math.PI * ((a - u) / (2 * a))) : -Math.sin(Math.PI * ((u - o) / (2 * (1 - o)))), h = (u) => r - u * (u >= 0 ? n : l), d = (u, x, _) => {
      ...
      for (let f = 0; f <= _; f++) {
        const B = u + (x - u) * f / _;
        v.push(`${(B * c).toFixed(2)},${h(p(B)).toFixed(2)}`);
      }
      return `M${v.join(" L")}`;
  ...
  _iconUrl(e) {
    ...
    return t ? `${t.replace(/\/+$/, "")}/${e}.png` : new URL((/* @__PURE__ */ Object.assign({ "../icons/clear.png": ge, "../icons/cloudy.png": ue, "../icons/drizzle.png": Ae, "../icons/fog.png": fe, "../icons/freezing_rain.png": me, "../icons/haze.png": we, "../icons/heavy_rain.png": ye, "../icons/heavy_snow.png": be, "../icons/night_clear.png": ve, "../icons/night_cloudy.png": xe, "../icons/night_drizzle.png": _e, "../icons/partly_cloudy.png": ke, "../icons/rain.png": Ce, "../icons/snow.png": Be, "../icons/sunrise.png": Oe, "../icons/sunset.png": Se, "../icons/thunderstorm.png": Ee, "../icons/windy.png": Me }))[`../icons/${e}.png`], import.meta.url).href;
  }
  // -- render ---------------------------------------------------------------
  render() {
  ...
  _renderHero(e) {
    const t = this._config.name ?? e.attributes.friendly_name ?? this._config.entity, i = this._override("temperature", "temperature"), s = this._daily[0], a = D(s?.temperature), o = D(s?.templow), r = Ut[e.state] ?? e.state, n = this._tap("hero"), l = this._heroScene(e.state), c = this._config, p = (g) => typeof g == "number" ? `${g}px` : g, h = [], d = (c.backgrounds_path ?? xi).replace(/\/+$/, "");
    return l && h.push(`--fwc-hero: url("${d}/hero-${l}.jpg?v=2")`), c.hero_bleed_x !== void 0 && h.push(`--fwc-hero-bleed-x: ${p(c.hero_bleed_x)}`), c.hero_bleed_top !== void 0 && h.push(`--fwc-hero-bleed-top: ${p(c.hero_bleed_top)}`), c.hero_extend !== void 0 && h.push(`--fwc-hero-extend: ${p(c.hero_extend)}`), c.hero_radius !== void 0 && h.push(`--fwc-hero-radius: ${p(c.hero_radius)}`), m`
  ...
  _renderHourly() {
    ...
    this._hourly.forEach((h, d) => {
      const g = new Date(h.datetime);
      g.getTime() > i || s.push({
        kind: "hour",
        time: g,
        label: d === 0 ? m`Now` : this._timeLabel(g),
        condition: h.condition ?? "",
        temp: D(h.temperature)
      });
    });
    const { rising: a, setting: o } = this._sunTimes();
    for (const [h, d] of [[a, "sunrise"], [o, "sunset"]])
      h && h.getTime() > t && h.getTime() < i && s.push({ kind: "sun", time: h, label: this._timeLabel(h, !0), event: d });
    s.sort((h, d) => h.time.getTime() - d.time.getTime());
    const r = this._override("wind_gust"), n = this._overrideUnit("wind_gust", "wind_speed_unit") ?? "km/h", l = Ut[this._weather?.state ?? ""] ?? "", c = l ? `${l} conditions expected for the rest of the day.` + (r !== void 0 ? ` Wind gusts are up to ${k(r)} ${n}.` : "") : "", p = this._tap("hourly");
    return m`
      <div class="panel strip" ?tappable=${!!p}
           @pointerdown=${this._markPointer} @click=${p}>
        ${c ? m`<div class="strip-summary">${c}</div>` : w}
        <div class="row">
          ${s.map((h) => h.kind === "sun" ? m`
                <div class="cell">
                ...
                <div class="cell">
                  <div class="cell-label">${h.label}</div>
                  <img class="cell-icon" src=${this._iconUrl(Q(h.condition, this._nightAt(h.time)))} alt=${h.condition} />
  ...
  _renderDaily() {
    ...
    const t = e.map((c) => D(c.templow)).filter((c) => c !== void 0), i = e.map((c) => D(c.temperature)).filter((c) => c !== void 0), s = Math.min(...t, ...i), a = Math.max(...t, ...i), o = Math.max(a - s, 1), r = this._override("temperature", "temperature"), n = this.hass?.locale?.language ?? navigator.language, l = this._tap("daily");
    return m`
      ...
      <div class="sheet-wrap" @click=${this._closeDaySheet}>
        ...
        <div class="sheet" @click=${(n) => n.stopPropagation()}>
          ...
          <div class="sheet-strip">
            ${t.map((n, l) => {
      const c = new Date(n.datetime);
      return m`
                <button class="sday ${l === e ? "on" : ""}"
                        @click=${() => this._openDaySheet(l)}>
                  <span class="sday-w">${c.toLocaleDateString(s, { weekday: "narrow" })}</span>
                  <span class="sday-n">${c.getDate()}</span>
                </button>
              `;
    })}
          </div>
  ...
  _renderSheetBody(e, t, i) {
    ...
    const g = e.length, A = (f) => g > 1 ? f / (g - 1) * 100 : 50, b = (f) => (p - f) / h * 100, u = e.map((f, B) => `${A(B)},${b(f.temp)}`).join(" "), x = `0,100 ${u} 100,100`, _ = Math.max(1, Math.round(g / 8)), v = this._hourScrub !== null ? e[this._hourScrub] : void 0;
    return m`
      ...
      <div class="sheet-glyphs">
        ${e.map((f, B) => B % _ === 0 ? m`<img class="sglyph ${B === 0 ? "first" : B === g - 1 ? "last" : ""}"
                      style=${`left:${A(B)}%`}
      ...
      this._hourScrub = null;
          ...
          <div class="smark lo ${v ? "muted" : ""}"
               style=${`left:${A(e.indexOf(n))}%; top:${b(o)}%`}>
            <span>L</span>
          </div>
          ${v ? m`
      ...
      <div class="sheet-xaxis">
        ${e.map((f, B) => f.hour % 6 === 0 ? m`<span class=${B === 0 ? "first" : B === g - 1 ? "last" : ""}
                       style=${`left:${A(B)}%`}>${this._hourLabel(f.hour)}</span>` : w)}
      </div>
    `;
  }
  /**
  ...
  _positionReadout() {
    ...
    e.style.transform = "translateX(0)";
    const o = e.getBoundingClientRect(), r = t.getBoundingClientRect(), n = i.getBoundingClientRect(), l = getComputedStyle(i), c = parseFloat(l.paddingLeft) || 0, p = parseFloat(l.paddingRight) || 0, h = r.left + a * r.width, d = n.left + c - o.left, g = n.right - p - o.width - o.left, A = Math.min(Math.max(h - o.width / 2 - o.left, d), Math.max(g, d));
  ...
  _renderSunTile(e, t, i) {
    ...
    return m`
      <div class="tile sun-tile" ?tappable=${!!l}
           @pointerdown=${this._markPointer} @click=${l}>
        <div class="tile-head">${z.sunrise} ${e ? "SUNRISE" : "SUNSET"}</div>
  ...
  _renderTiles() {
    ...
    return m`
        ...
        <div class="tile" ?tappable=${!!this._tap("feels_like")}
          ...
          <div class="tile-head">${z.thermometer} FEELS LIKE</div>
        ...
        <div class="tile" ?tappable=${!!this._tap("humidity")}
          ...
          <div class="tile-head">${z.humidity} HUMIDITY</div>
          <div class="tile-value">${k(a)}%</div>
          <div class="tile-note">
            ${o !== void 0 ? m`The dew point is ${k(o)}° right now.` : w}
          </div>
        </div>
  ...
  _renderMapTile() {
    ...
    const n = e ? this._mapSeries : void 0, l = n ? Math.min(this._mapFrame, n.frames.length - 1) : 0, c = n ? new Date(n.times[l]) : void 0, p = this._config?.map_style ?? "dark", h = t ? li(t, p) : [];
    return m`
      ...
      this._mapOpen || this._toggleMap();
        ...
        <div class="map-frame">
          <div class="map-base">
            ${h.map((d) => m`
              <img class="map-tile" src=${d.base} alt="" style=${`left:${d.left}px;top:${d.top}px;width:${d.size}px;height:${d.size}px`} />`)}
          </div>
          ...
          <div class="map-labels">
            ${h.map((d) => m`
              <img class="map-tile" src=${d.ref} alt="" style=${`left:${d.left}px;top:${d.top}px;width:${d.size}px;height:${d.size}px`} />`)}
          </div>
          ...
          ${this._mapOpen ? m`
            <div class="map-legend" @click=${(d) => d.stopPropagation()}>
            ...
            <button class="map-close" title="Close"
                    @click=${(d) => {
      d.stopPropagation(), this._toggleMap();
    }}>
          ...
          ${this._mapOpen && !n?.frames.length ? m`
            <div class="map-bar no-data" @click=${(d) => d.stopPropagation()}>
          ...
          ${this._mapOpen && n?.frames.length && c ? m`
            <div class="map-bar" @click=${(d) => d.stopPropagation()}>
      ...
      return !A && g % 2 !== 0 ? w : m`<span>
          ...
          ${this._mapOpen ? m`<div class="map-credit">${hi}</div>` : w}
        </div>
      </div>
    `;
  }
  ...
  _renderWindTile(e, t, i, s, a) {
    const r = (M) => (M - 90) * Math.PI / 180, n = (M, $) => [48 + M * Math.cos($), 48 + M * Math.sin($)];
    let l = "";
    for (let M = 0; M < 72; M++) {
      const $ = M * 5;
    ...
    const c = a !== void 0, p = c ? a + 180 : 0, h = r(p), d = r(p + 180), g = 38, [A, b] = n(g, d), [u, x] = n(g - 3, d), [_, v] = n(24.5, d), [f, B] = n(22.5, h), [Zt, te] = n(30, h), ct = 1.32, wt = (g + 2 + 30) / 2, yt = (g + 2 - 30) / 2 * ct, [ee, ie] = n(wt + yt, h), [bt, vt] = n(wt - yt, h), xt = -Math.sin(h), _t = Math.cos(h), et = 3.4 * ct, se = bt + xt * et, ae = vt + _t * et, oe = bt - xt * et, re = vt - _t * et, ne = (M) => {
    ...
    return m`
      <div class="tile wide" ?tappable=${!!kt}
        ...
        <div class="tile-head">${z.wind} WIND</div>
        <div class="wind-body">
          <div class="wind-rows">
          ...
          <svg class="dial" viewBox="0 0 96 96" aria-hidden="true">
            ...
            ${c ? T`
              ...
              <line class="dial-shaft" x1=${f.toFixed(2)} y1=${B.toFixed(2)}
...
S([
  E()
], y.prototype, "_hourly", 2);
S([
  E()
], y.prototype, "_daily", 2);
S([
  E()
], y.prototype, "_grid", 2);
S([
  E()
], y.prototype, "_mapOpen", 2);
S([
  E()
], y.prototype, "_mapFrame", 2);
S([
  E()
], y.prototype, "_mapPlaying", 2);
S([
  E()
], y.prototype, "_sheetDay", 2);
S([
  E()
], y.prototype, "_hourScrub", 2);
S([
  E()
], y.prototype, "_hourlyDays", 2);
S([
  E()
], y.prototype, "_hourlyError", 2);
S([
  E()
], y.prototype, "_mapRange", 2);
S([
  E()
